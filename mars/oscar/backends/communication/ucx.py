...
class UCXInitializer:
    ...
    def _get_options(ucx_config: dict) -> Tuple[dict, dict]:
        ...
        if any(ucx_config.get(name) for name in ["tcp", "nvlink", "infiniband"]):
            ...
            if any(ucx_config.get(name) for name in ["nvlink", "cuda-copy"]):
                tls += ",cuda_copy"

            if ucx_config.get("infiniband"):
                tls = "rc," + tls
            if ucx_config.get("nvlink"):
                tls += ",cuda_ipc"
        ...
        for k, v in ucx_config.get("environment", dict()).items():
...
class UCXChannel(Channel):
    ...
    def closed(self):
        if self._has_close_callback is None:
